...
namespace AssetAutomator.Services
{
    ...
    public class GeminiCreatorService
    {
        ...
        public async Task<(bool Success, string Message)> ImportCookiesAsync(
            ...
            Action<string>? onStatus = null)
        {
            var syncService = new GeminiCookieSyncService(msg => _log.Info(LogCategory.CookieSync, msg));
            bool imported = false;
            ...
            Action<string>? onStatus = null)
        {
            try
            {
                string profilesDir = ConfigService.CurrentSettings.ChromeProfilesDir;
