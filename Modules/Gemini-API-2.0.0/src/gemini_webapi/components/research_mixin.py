...
class ResearchMixin:
    ...
    async def start_deep_research(
        ...
        if output.deep_research_plan:
            if output.deep_research_plan.research_id:
                plan.research_id = output.deep_research_plan.research_id
            if output.deep_research_plan.cid:
                plan.cid = output.deep_research_plan.cid
        return output
    ...
    async def wait_for_deep_research(
        ...
        if not plan.research_id and not plan.cid:
            raise GeminiError(
                "Cannot poll deep research status: both plan.research_id and plan.cid are missing. "
        ...
        final_output = None
        cid = plan.cid or (statuses[-1].cid if statuses else None)
        if cid:
            # Poll latest chat response if final result not yet ready
            while (time.time() - start) < timeout:
                final_output = await self.fetch_latest_chat_response(cid)
                if final_output and final_output.text:
                    # Check if Gemini has completed the full research report (more than confirmation text)
                    if len(final_output.text) > 300 and not final_output.text.strip().startswith("Tuyệt!"):
                        break
                await asyncio.sleep(poll_interval)

        done = bool(statuses and statuses[-1].done) or bool(final_output and len(final_output.text or "") > 300)
