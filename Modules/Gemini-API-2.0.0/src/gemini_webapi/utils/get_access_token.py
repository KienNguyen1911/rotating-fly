...
async def get_access_token(
    ...
    # Phase 1: Base Cookies (Explicitly provided by user)
    if base_psid:
        ...
        if cache_file and cache_file.is_file():
            ...
            if content:
                try:
                    ...
                    cached_jar.update(extra_cookies)
                    cached_psidts = _extract_cookie_value(cached_jar, "__Secure-1PSIDTS") or ""
                    if cached_psidts not in tried_sessions.get(base_psid, set()):
                        cookie_jars_to_test.append((cached_jar, "Cache"))
                        tried_sessions.setdefault(base_psid, set()).add(cached_psidts)
