...
class ClusterManager:
    def __init__(self, db: Database, runtime):
        ...
        # Record short-term dispatch reservation slots to cover the "heartbeat reporting lag" window and avoid instantaneous over-dispatching.
        self._dispatch_reservations: dict[int, list[float]] = {}
        # Record short-term dispatch affinity from bucket -> node to keep similar requests routed stably to the same child node.
        ...
        # "Dispatched but not yet finished" routed sessions maintained locally by the master node, used to idempotently reclaim active_sessions.
    ...
    @staticmethod
    def _normalize_dispatch_bucket_key(bucket_key: str | None) -> str:
    ...
    def _prune_dispatch_bucket_affinity_locked(self, *, valid_node_ids: set[int] | None = None):
    ...
    async def _mark_bucket_affinity(self, bucket_key: str | None, node_id: int):
    ...
    async def _clear_bucket_affinity(self, bucket_key: str | None, *, expected_node_id: int = 0):
        normalized = self._normalize_dispatch_bucket_key(bucket_key)
        if not normalized:
            return
    ...
    async def dispatch_solve(self, request_payload: dict[str, Any]) -> dict[str, Any]:
        ...
        while True:
            ...
            for node in nodes:
                ...
                try:
                    ...
                    if not child_session or not token:
                        raise RuntimeError("Child node response is missing session_id/token")

                    routed_session_id = f"{node['id']}:{child_session}"
                    # After a successful solve, immediately release temporary reservations and sync active_sessions to avoid occupying capacity falsely for too long.
            ...
            if fatal_error is not None:
                raise RuntimeError(f"Child node dispatch failed: {fatal_error}") from fatal_error
    ...
    async def _resolve_routed_session(self, routed_session_id: str) -> tuple[dict[str, Any], str]:
        ...
        if ":" not in raw:
            raise RuntimeError("In master mode, session_id must be nodeId:childSessionId")
        node_part, child_session = raw.split(":", 1)
        if not node_part.isdigit() or not child_session:
            raise RuntimeError("Invalid session_id routing format")

        node_id = int(node_part)
        node = await self.db.get_cluster_node(node_id)
        if not node:
            raise RuntimeError("Routing node does not exist")
        if not bool(node.get("enabled", 0)):
            raise RuntimeError("Routing node is disabled")
        ...
        *,
        ...
        for node in nodes:
            ...
            if parsed.scheme not in {"http", "https"} or not host:
                debug_logger.log_warning(
                    f"[ClusterManager] Skipping invalid child node address node={node.get('node_name')} base_url={base_url}"
                )
                continue
            if host in {"0.0.0.0", "127.0.0.1", "localhost", "::1", "::"}:
                debug_logger.log_warning(
                    f"[ClusterManager] Skipping unreachable child node address node={node.get('node_name')} base_url={base_url}"
                )
                continue
        ...
        async with self._dispatch_lock:
            ...
            if not with_idle:
                # If all nodes have no free capacity, return empty directly to avoid sending traffic to full nodes.
                return []
    ...
    def _prune_completed_routed_sessions_locked(self, now_ts: float | None = None):
    ...
    @staticmethod
    def _parse_db_timestamp(raw: Any) -> datetime | None:
    ...
    @classmethod
    def _summarize_last_error(cls, last_error: str) -> tuple[str, str] | None:
        ...
        if "cluster key" in lowered_error or "api key" in lowered_error or "unauthorized" in lowered_error or "forbidden" in lowered_error or "auth" in lowered_error or "invalid" in lowered_error:
        ...
        if "connection refused" in lowered_error or "failed to establish a new connection" in lowered_error:
            return "report_failed", "Connection failed"

        if "node_not_registered" in lowered_error:
            return "report_failed", "Node not registered"

        if "timed out" in lowered_error or "timeout" in lowered_error:
            return "timeout", "Request timeout"

        if "heartbeat failed" in lowered_error or "register failed" in lowered_error or "report" in lowered_error:
            return "report_failed", "Reporting failed"
    ...
    def _classify_health_reason(
        *,
        ...
        if heartbeat_age_seconds is None or heartbeat_age_seconds > stale_seconds:
            return "timeout", "Heartbeat timeout"
    ...
    async def _post_to_node(
        ...
        if not base_url or not api_key:
            raise RuntimeError("Node configuration is missing base_url or node_api_key")
        ...
        if isinstance(payload, dict):
            return payload
        raise RuntimeError("Child node response is not a JSON object")
    ...
    def _sync_json_http_request(
        ...
        try:
            ...
            debug_logger.log_warning(
                f"[ClusterManager] HTTPS certificate verification failed, retrying internal node communication in non-verifying mode: {url}"
            )
            try:
                ...
                raise RuntimeError(f"HTTP request failed: {retry_error}") from retry_error
    ...
    async def _send_subnode_heartbeat(self):
        ...
        if not master_base or not cluster_key or not node_api_key:
            debug_logger.log_warning(
                "[ClusterManager] subnode mode is missing master_base_url/master_cluster_key/node_api_key, skipping heartbeat"
            )
            return

        public_base_url = str(config.cluster_node_public_base_url or "").strip()
        if not public_base_url:
            debug_logger.log_warning(
                "[ClusterManager] subnode mode is missing node_public_base_url, skipping heartbeat. "
                "Please fill in the child node address accessible by the master node, e.g., http://subnode:8060 or http://public_IP:8061"
            )
            return
        ...
        if parsed_public.scheme not in {"http", "https"} or not public_host:
            debug_logger.log_warning(
                f"[ClusterManager] node_public_base_url is invalid: {public_base_url}"
            )
            return

        if public_host in {"0.0.0.0", "127.0.0.1", "localhost", "::1", "::"}:
            debug_logger.log_warning(
                "[ClusterManager] node_public_base_url cannot be 0.0.0.0 / 127.0.0.1 / localhost. "
