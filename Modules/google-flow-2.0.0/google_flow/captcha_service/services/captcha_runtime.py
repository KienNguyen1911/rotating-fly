...
from .session_registry import SessionEntry, SessionRegistry
...
class CaptchaRuntime:
    ...
    async def _get_browser_service(self):
        if config.cluster_role == "master":
            raise RuntimeError("Master role does not perform local captcha solving")
        ...
        async with self._browser_service_lock:
            ...
            if self._browser_service is None:
                if current_mode == "personal":
                    from .browser_captcha_personal import (
                        BrowserCaptchaService as PersonalCaptchaService,
                    )
    ...
    @staticmethod
    def _extract_token_response(raw_result: Any, fallback_browser_id: str | None = None) -> tuple[str | None, str | None, dict[str, Any] | None]:
    ...
    async def solve(
        ...
        if not token or browser_id is None:
            raise RuntimeError("Headed captcha solving failed, token not obtained")
    ...
    async def custom_token(
        ...
        if self._service_mode == "personal":
            ...
            token, browser_id, fingerprint = self._extract_token_response(token_result)
        if not token:
            raise RuntimeError("General captcha solving failed, token not obtained")
    ...
    async def finish(self, session_id: str) -> tuple[bool, str, SessionEntry | None]:
    ...
    async def mark_error(self, session_id: str, error_reason: str) -> tuple[bool, str, SessionEntry | None]:
        entry = await self.registry.get(session_id)
        if not entry:
