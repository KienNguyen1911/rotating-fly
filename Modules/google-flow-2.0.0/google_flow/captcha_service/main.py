...
async def startup_captcha_service() -> None:
...
async def shutdown_captcha_service() -> None:
...
async def root(request: Request):
    ...
    if "text/html" in accept:
        ...
        return _static_page(filename, "Public page does not exist")
...
async def subnode_page():
    return _static_page("subnode.html", "The child node page does not exist")


@app.get("/admin", include_in_schema=False)
async def admin_panel():
    return _static_page("admin.html", "The admin panel page does not exist")


def main():
    import uvicorn
    uvicorn.run(
        "google_flow.captcha_service.main:app",
