...
def _resolve_service_request_owner(api_key: dict) -> tuple[int | None, int | None, int | None]:
...
async def prefill_solve_pool(
    ...
    if config.cluster_role == "master":
        raise HTTPException(status_code=400, detail="The master role does not support local prefill")
...
async def custom_token(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"custom-token failed: {e}")
