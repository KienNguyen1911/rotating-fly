...
_task_manager: YesCaptchaTaskManager | None = None
...
def _resolve_owner_ids(api_key: dict[str, Any]) -> tuple[int | None, int | None, int | None]:
    portal_user_id = int(api_key.get("portal_user_id") or 0)
    portal_api_key_id = int(api_key.get("portal_api_key_id") or 0)
    if portal_user_id > 0:
...
async def _read_json_body(request: Request) -> dict[str, Any]:
    try:
        ...
        raise YesCaptchaProtocolError("ERROR_BAD_REQUEST", f"Request body is not valid JSON: {exc}") from exc
    if not isinstance(payload, dict):
        raise YesCaptchaProtocolError("ERROR_BAD_REQUEST", "The request body must be a JSON object")
    return payload
...
async def _resolve_client_key(client_key: str) -> dict[str, Any]:
    ...
    try:
        ...
        async with _client_key_cache_lock:
            _client_key_cache.pop(normalized_key, None)
        detail = str(exc.detail or "").strip() or "API Key is invalid"
        status_code = int(exc.status_code or 401)
        if status_code == 403:
            if "Disable" in detail:
...
async def _ensure_available(api_key: dict[str, Any]):
    if _db is None:
        raise YesCaptchaProtocolError("ERROR_INTERNAL", "Database not initialized")
    ...
    if "exhausted" in normalized:
        raise YesCaptchaProtocolError("ERROR_ZERO_BALANCE", normalized or "Insufficient quota")
    if "Disable" in normalized:
        raise YesCaptchaProtocolError("ERROR_KEY_DISABLED", normalized or "API Key is disabled")
    raise YesCaptchaProtocolError("ERROR_ACCESS_DENIED", normalized or "The current account is unavailable")
...
async def _consume_quota(api_key: dict[str, Any], task_id: str):
    ...
    if "exhausted" in normalized:
        raise YesCaptchaProtocolError("ERROR_ZERO_BALANCE", normalized or "Insufficient quota")
    raise YesCaptchaProtocolError("ERROR_ACCESS_DENIED", normalized or "Credit deduction failed")
...
async def _query_balance(api_key: dict[str, Any]) -> float:
    ...
    if portal_user_id > 0:
        ...
        if not user:
            raise YesCaptchaProtocolError("ERROR_KEY_DOES_NOT_EXIST", "User does not exist")
        remaining = user.get("quota_remaining")
    else:
        fresh_api_key = await _db.get_api_key(int(api_key.get("id") or 0))
        if not fresh_api_key:
            raise YesCaptchaProtocolError("ERROR_KEY_DOES_NOT_EXIST", "API Key does not exist")
...
def _normalize_task(task_payload: Any) -> dict[str, Any]:
    if not isinstance(task_payload, dict):
        raise YesCaptchaProtocolError("ERROR_BAD_PARAMETERS", "task must be an object")

    task_type = str(task_payload.get("type") or "").strip()
    if not task_type:
        raise YesCaptchaProtocolError("ERROR_BAD_PARAMETERS", "task.type cannot be empty")
    mapping = _TASK_TYPE_MAPPING.get(task_type)
    if mapping is None:
        raise YesCaptchaProtocolError("ERROR_TASK_NOT_SUPPORTED", f"Currently only {', '.join(_TASK_TYPE_MAPPING.keys())} is supported")

    website_url = str(task_payload.get("websiteURL") or "").strip()
    website_key = str(task_payload.get("websiteKey") or "").strip()
    if not website_url:
        raise YesCaptchaProtocolError("ERROR_BAD_PARAMETERS", "task.websiteURL cannot be empty")
    if not website_key:
        raise YesCaptchaProtocolError("ERROR_BAD_PARAMETERS", "task.websiteKey cannot be empty")
...
def _extract_user_agent(fingerprint: Any) -> str | None:
...
async def _solve_custom_token(task_payload: dict[str, Any]) -> dict[str, Any]:
    if _runtime is None:
        raise YesCaptchaProtocolError("ERROR_INTERNAL", "Not initialized at runtime")

    if config.cluster_role == "master":
        if _cluster is None:
            raise YesCaptchaProtocolError("ERROR_INTERNAL", "cluster manager not initialized")
...
async def _process_task(task_id: str, owner_scope: str, api_key: dict[str, Any], task_payload: dict[str, Any]):
    ...
    try:
        ...
        if not token:
            raise YesCaptchaProtocolError("ERROR_CAPTCHA_UNSOLVABLE", "No valid token was obtained")
        ...
        error = YesCaptchaProtocolError("ERROR_CAPTCHA_UNSOLVABLE", str(exc) or "Task execution failed")
        await _task_manager.mark_error(
            task_id,
            owner_scope=owner_scope,
...
def _task_result_payload(record: YesCaptchaTaskRecord) -> dict[str, Any]:
    ...
    return _error_response(
        ...
        record.error_description or "Task execution failed",
...
async def create_task(request: Request):
    try:
        if _task_manager is None:
            raise YesCaptchaProtocolError("ERROR_INTERNAL", "Task manager is not initialized")

        payload = await _read_json_body(request)
        client_key = str(payload.get("clientKey") or "").strip()
        if not client_key:
            raise YesCaptchaProtocolError("ERROR_KEY_DOES_NOT_EXIST", "clientKey cannot be empty")
        ...
        return JSONResponse(_error_response("ERROR_INTERNAL", str(exc) or "createTask failed"))
...
async def get_task_result(request: Request):
    try:
        ...
        if not client_key:
            raise YesCaptchaProtocolError("ERROR_KEY_DOES_NOT_EXIST", "clientKey cannot be empty")
        if not task_id:
            raise YesCaptchaProtocolError("ERROR_BAD_PARAMETERS", "taskId cannot be empty")

        api_key = await _resolve_client_key(client_key)
        record = await _task_manager.get_task(task_id, owner_scope=_owner_scope(api_key))
        if record is None:
            raise YesCaptchaProtocolError("ERROR_TASK_NOT_FOUND", "taskId does not exist or has no permission to access")
        ...
        return JSONResponse(_error_response("ERROR_INTERNAL", str(exc) or "getTaskResult failed"))
...
async def get_balance(request: Request):
    try:
        ...
        return JSONResponse(_error_response("ERROR_INTERNAL", str(exc) or "getBalance failed"))
