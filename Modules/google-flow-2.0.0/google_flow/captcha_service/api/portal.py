...
def _assert_portal_public_role(feature_name: str):
    if config.cluster_role == "subnode":
        raise HTTPException(status_code=400, detail=f"The current subnode role is not open: {feature_name}")


def _assert_local_portal_auth_enabled(feature_name: str):
    if config.portal_oauth_only:
        raise HTTPException(status_code=400, detail=f"Currently only OAuth/OIDC login is allowed, closed: {feature_name}")
...
def _sanitize_oidc_headers(headers: dict[str, str] | None) -> dict[str, str]:
...
def _sanitize_oidc_body(body: bytes | None) -> str:
    if not body:
        return ""
...
def _build_oidc_browser_headers(extra_headers: dict[str, str] | None = None) -> dict[str, str]:
    headers = {
...
async def _oidc_http_request(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"OIDC upstream request failed: {exc}")
    ...
    if "application/json" in lowered_content_type:
        try:
            ...
            raise HTTPException(status_code=502, detail="OIDC upstream returned invalid JSON")
...
async def _request_oidc_token(
    ...
    auth_value = base64.b64encode(f"{client_id}:{client_secret}".encode()).decode("ascii")
    attempts = [
        {
    ...
    last_error_detail = "OIDC token response is invalid"
    for attempt in attempts:
        ...
        if 200 <= token_status < 300 and isinstance(token_payload, dict):
            ...
            last_error_detail = "OIDC token response is missing access_token"
        else:
            debug_logger.log_warning(
                "[portal_oidc] token exchange response "
                f"mode={attempt['name']} status={token_status} "
            ...
            last_error_detail = (
                f"OIDC token response exception status={token_status}"
                if not isinstance(token_payload, dict)
                else str(token_payload.get("error_description") or token_payload.get("error") or f"OIDC token response exception status={token_status}")
            )
        debug_logger.log_warning(
...
async def _resolve_oidc_endpoints(settings: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise HTTPException(status_code=502, detail="OIDC well-known response is invalid")
    ...
    if missing_fields:
        raise HTTPException(
            status_code=502,
            detail=f"OIDC well-known missing fields: {', '.join(missing_fields)}",
        )
...
async def _resolve_or_create_oidc_user(claims: dict[str, Any], provider_base_url: str) -> dict[str, Any]:
    ...
    if not ok or not user:
        raise HTTPException(status_code=409, detail=message or "OIDC user creation failed")
    return user
...
def _build_cluster_summary(cluster_stats: dict[str, Any]) -> dict[str, Any]:
    if config.cluster_role != "master":
        return {
            ...
            "nodes": [],
            "message": "The current role is not master, and the client does not display the child node scheduling dashboard.",
        }
        ...
        nodes.append(
            {
                ...
                "health_reason": node.get("health_reason") or "unknown",
    ...
    return {
        ...
        "nodes": nodes,
        "message": "Only the user-side read-only summary is displayed, and administrator-sensitive configurations are not included.",
    }
...
def _build_quickstart(base_url: str) -> dict[str, Any]:
    if config.cluster_role == "subnode":
        return {
            ...
            "endpoints": [
                {"name": "Child node status page", "method": "GET", "path": "/", "description": "The sub-node homepage only displays node status, health check and backend entrance, and does not provide user registration/login."},
                {"name": "health check", "method": "GET", "path": "/api/v1/health", "description": "Used for container health detection, node connectivity confirmation and deployment troubleshooting."},
                {"name": "Service coding", "method": "POST", "path": "/api/v1/solve", "description": "It can be used for internal joint debugging. Production clusters are usually scheduled and forwarded to subnodes by the master."},
                {"name": "Management background", "method": "GET", "path": "/admin", "description": "Administrators can view the running configuration, logs, and execution status of this node."},
            ],
        }
    ...
    return {
        ...
        "console_actions": ["solve", "finish", "error", "custom-score"],
        "endpoints": [
            {"name": "User registration", "method": "POST", "path": "/api/portal/auth/register", "description": "User self-registration, verify registration location and duplicate user name."},
            {"name": "User login", "method": "POST", "path": "/api/portal/auth/login", "description": "After logging in, you get the portal token, and there is no need to expose administrator data when coding on the client side."},
            {"name": "Redeem CDK", "method": "POST", "path": "/api/portal/cdks/redeem", "description": "After successful redemption, the number of available coding times will be increased."},
            {"name": "User coding", "method": "POST", "path": "/api/portal/user/solve", "description": "Verify the available number of times first; the number will be deducted only if the final generation is successful, and the number of times will be returned if it fails."},
            {"name": "Advanced API debugging", "method": "POST", "path": "/api/v1/solve", "description": "Advanced access still supports API Key direct connection mode."},
        ],
    }
...
def _resolve_register_location(request: Request, register_location: str) -> str:
    ...
    if normalized_location not in allowed_locations:
        raise HTTPException(status_code=400, detail="Registration location is invalid")
    ...
    if referer:
        ...
        if referer_path not in {"/", "/portal"}:
            raise HTTPException(status_code=400, detail="Registration location verification failed")
...
async def _cleanup_portal_success_result(result: dict[str, Any] | None):
    if not result:
        return
...
async def portal_user_register(request: PortalRegisterRequest, raw_request: Request, response: Response):
    _assert_portal_public_role("User self-registration")
    _assert_local_portal_auth_enabled("User self-registration")
    if _db is None:
        raise HTTPException(status_code=500, detail="Service not initialized")

    confirm_password = getattr(request, "confirm_password", None)
    if confirm_password and request.password != confirm_password:
        raise HTTPException(status_code=400, detail="The two password inputs are inconsistent")
...
async def portal_user_login(request: LoginRequest, response: Response):
    _assert_portal_public_role("User login")
    _assert_local_portal_auth_enabled("Login with username and password")
    if _db is None:
        raise HTTPException(status_code=500, detail="Service not initialized")
    ...
    token = await _login_portal_user(response, int(user["id"]))
    await _build_portal_user_workspace_payload(int(user["id"]))
    return {
        "success": True,
...
async def portal_oidc_start(request: Request):
    _assert_portal_public_role("OIDC login")
    settings = await _resolve_oidc_endpoints(_get_oidc_settings())
    if not settings["enabled"]:
        raise HTTPException(status_code=400, detail="OIDC login is not currently enabled")
...
async def portal_oidc_callback(
    ...
    portal_oidc_state: str | None = Cookie(default=None),
):
    _assert_portal_public_role("OIDC login")
    settings = await _resolve_oidc_endpoints(_get_oidc_settings())
    portal_path = "/portal"
    if not settings["enabled"]:
        return RedirectResponse(url=f"{portal_path}?oidc_error={urllib.parse.quote('OIDC login is not currently enabled')}", status_code=302)
    if error:
        return RedirectResponse(url=f"{portal_path}?oidc_error={urllib.parse.quote(error)}", status_code=302)
    if not code:
        return RedirectResponse(url=f"{portal_path}?oidc_error={urllib.parse.quote('OIDC callback missing code')}", status_code=302)
    if not state or not portal_oidc_state or state != portal_oidc_state:
        return RedirectResponse(url=f"{portal_path}?oidc_error={urllib.parse.quote('OIDC state verification failed')}", status_code=302)
    ...
    access_token = str(token_payload.get("access_token") or "").strip()
    if not access_token:
        raise HTTPException(status_code=502, detail="OIDC token response is missing access_token")
    ...
    if userinfo_status < 200 or userinfo_status >= 300 or not isinstance(userinfo_payload, dict):
        raise HTTPException(status_code=502, detail="OIDC userinfo response is invalid")
...
async def portal_check_username(username: str = Query(min_length=1, max_length=60)):
    _assert_portal_public_role("Registration check")
    if _db is None:
        raise HTTPException(status_code=500, detail="Service not initialized")

    user = await _db.get_portal_user_by_username(username)
    if not user:
        return {"success": True, "registered": False, "message": "This username has not been registered yet"}

    return {
        "success": True,
        "registered": True,
        "message": "This username has been registered, please log in directly",
        "user": {
...
async def create_portal_user_api_key(
    ...
    return {"success": True, "api_key": raw_key, "item": item, "message": "Only the complete API Key will be returned this time, please save it now"}
...
async def soft_delete_portal_user_api_key(api_key_id: int, user: dict = Depends(verify_portal_user_token)):
    ...
    if not item:
        raise HTTPException(status_code=404, detail="API Key does not exist")
    return {"success": True, "item": item, "message": f"API Key #{api_key_id} soft deleted (disabled)"}
...
async def portal_redeem_cdk(request: PortalRedeemRequest, user: dict = Depends(verify_portal_user_token)):
    _assert_portal_public_role("CDK exchange")
    if _db is None:
        raise HTTPException(status_code=500, detail="Service not initialized")
...
async def portal_user_solve(request: SolveRequest, user: dict = Depends(verify_portal_user_token)):
    _assert_portal_public_role("User coding")
    if _db is None or _runtime is None:
        raise HTTPException(status_code=500, detail="Service not initialized")
    ...
    try:
        if config.cluster_role == "master":
            if _cluster is None:
                raise RuntimeError("cluster manager not initialized")
        ...
        raise HTTPException(status_code=500, detail=f"Coding failed: {e}")
...
async def portal_user_finish_session(
    ...
    if config.cluster_role == "master" and ":" in session_id:
        if _cluster is None:
            raise HTTPException(status_code=500, detail="cluster manager not initialized")
        try:
            ...
            raise HTTPException(status_code=500, detail=f"finish forwarding failed: {e}")
    else:
        try:
            ...
            raise HTTPException(status_code=500, detail=f"finish local processing failed: {e}")
        if not ok:
...
async def portal_user_report_error(
    ...
    if config.cluster_role == "master" and ":" in session_id:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"error forwarding failed: {e}")
    else:
        try:
            ...
            raise HTTPException(status_code=500, detail=f"error local processing failed: {e}")
        if not ok:
            raise HTTPException(status_code=404, detail=message)
...
async def portal_user_custom_score(
    ...
    _assert_portal_public_role("Custom score debugging")
    if _db is None or _runtime is None:
        raise HTTPException(status_code=500, detail="Service not initialized")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"custom-score failed: {e}")
