...
def _choose_internal_port(public_port: int) -> int:
    ...
    raise RuntimeError("Unable to allocate available port for internal uvicorn")
...
class _BridgeRequestHandler(BaseHTTPRequestHandler):
    ...
    def _read_chunked_request_body(self) -> bytes:
        ...
        while True:
            ...
            if not size_line:
                raise ValueError("chunked body missing chunk size")
            chunk_size_text = size_line.split(b";", 1)[0].strip()
            try:
                chunk_size = int(chunk_size_text, 16)
            except ValueError as exc:
                raise ValueError(f"Illegal chunk size: {chunk_size_text!r}") from exc
            ...
            if len(chunk) != chunk_size:
                raise ValueError("chunked body length is insufficient")
            chunks.append(chunk)

            chunk_end = self.rfile.readline()
            if chunk_end not in {b"\r\n", b"\n"}:
                raise ValueError("chunked body missing chunk terminator")
...
class BridgedUvicornRunner:
    ...
    def start(self):
        ...
        while time.monotonic() < deadline:
            ...
            time.sleep(0.05)
        raise RuntimeError(
            f"Internal uvicorn startup failed host={self._uvicorn_server.config.host} port={self._uvicorn_server.config.port}"
        )
    ...
    def serve_forever(self):
        ...
        while self._bridge_thread.is_alive():
            if not self._uvicorn_thread.is_alive():
                raise RuntimeError(
                    f"Internal uvicorn has exited host={self._uvicorn_server.config.host} port={self._uvicorn_server.config.port}"
                )
