...
class Database:
    ...
    def __init__(self, db_path: Path | None = None):
        ...
        self._redis_log_store: RedisLogStore | None = None
        self._log_cleanup_task: asyncio.Task | None = None
    ...
    async def initialize_log_store(self):
        ...
        if not redis_url:
            raise RuntimeError("Redis log storage is enabled but not configured log.redis_url / FCS_LOG_REDIS_URL")
    ...
    async def _create_job_log_in_tx(
        ...
        *,
        ...
        portal_api_key_id: int | None = None,
    ):
    ...
    async def _refund_portal_user_quota_in_tx(
        ...
        *,
        ...
        if not charged:
            return False, "No refundable deduction record found"
        ...
        if int(cursor.rowcount or 0) <= 0:
            return True, "The number of times this session has been returned"
        ...
        await db.execute(
            ...
            (
                ...
                "generation_refund",
                session_id,
                (reason or "Number of failed returns")[:200],
            ),
        )
    ...
    async def init_db(self):
        async with self._write_connect() as db:
            ...
            # In historical versions, node_name is set to UNIQUE, which will cause the sub-node with the same name to be overwritten; here it is uniformly migrated to non-unique.
    ...
    async def update_admin_credentials(
        ...
        return True, "Administrator account updated successfully", profile

    async def get_portal_user(self, user_id: int) -> dict[str, Any] | None:
        async with self._connect() as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute(
                """
    ...
    async def get_portal_user_by_username(self, username: str) -> dict[str, Any] | None:
    ...
    async def create_portal_user(
        ...
        if not normalized_location:
            return False, "Registration location cannot be empty", None
        existing = await self.get_portal_user_by_username(normalized_username)
        if existing:
            return False, "This username has already been registered", None
        ...
        return True, "Registration successful", await self.get_portal_user(user_id)

    async def verify_portal_user_credentials(self, username: str, password: str) -> dict[str, Any] | None:
    ...
    async def update_portal_user(
        ...
        if effective_delta != 0:
            await self.create_portal_user_transaction(
                ...
                source_ref=str(user_id),
                note="Administrator adjusts remaining times" if quota_remaining is None else "Administrator sets remaining times",
            )
        if enabled is False:
    ...
    async def ensure_portal_user_available(self, user_id: int) -> tuple[bool, str]:
        ...
        if int(user.get("quota_remaining") or 0) <= 0:
            return False, "Not enough times remaining"
        return True, ""
    ...
    def _normalize_redis_job_log(self, raw: Any) -> dict[str, Any] | None:
    ...
    async def _get_redis_job_logs(self, *, limit: int | None = None, offset: int = 0) -> list[dict[str, Any]]:
    ...
    async def list_portal_user_jobs(
        ...
        if self._job_logs_use_redis():
            ...
            portal_key_cache: dict[int, dict[str, Any] | None] = {}
    ...
    async def refund_stale_session_quotas(
        ...
        for item in portal_candidates:
            ...
            if not refunded or message != "Returned 1 time":
                continue
    ...
    async def get_portal_user_usage_summary(self, user_id: int) -> dict[str, Any] | None:
        user = await self.get_portal_user(user_id)
        if not user:
            return None
    ...
    async def claim_portal_user_checkin(self, user_id: int, min_quota: int, max_quota: int) -> tuple[bool, str, dict[str, Any]]:
        ...
        if safe_max <= 0:
            return False, "Sign-in rewards are currently not enabled", {"granted_quota": 0}
        ...
        async with self._write_connect() as db:
            ...
            try:
                ...
                if not user:
                    await db.execute("ROLLBACK")
                    return False, "User does not exist", {"granted_quota": 0}
                if not bool(user["enabled"]):
                    await db.execute("ROLLBACK")
                    return False, "User disabled", {"granted_quota": 0}
                ...
                await db.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (user_id, granted_quota, balance_after, "daily_checkin", None, "Daily sign-in rewards"),
                )
                await db.commit()
    ...
    async def update_portal_cdk(self, cdk_id: int, enabled: bool | None = None) -> dict[str, Any] | None:
        async with self._connect() as db:
            db.row_factory = aiosqlite.Row
    ...
    async def redeem_portal_cdk(self, user_id: int, code: str) -> tuple[bool, str, dict[str, Any] | None]:
        normalized_code = str(code or "").strip().upper()
        if not normalized_code:
            return False, "Redemption code cannot be empty", None
        ...
        async with self._connect() as db:
            ...
            try:
                ...
                if not cdk:
                    await db.execute("ROLLBACK")
                    return False, "Redemption code does not exist", None
                if not bool(cdk["enabled"]):
                    await db.execute("ROLLBACK")
                    return False, "Redeem code is disabled", None
                if cdk["redeemed_user_id"] is not None:
                    await db.execute("ROLLBACK")
                    return False, "The redemption code has been used", None
        ...
        return True, "Redemption successful", {"user": updated_user, "cdk": redeemed}
    ...
    async def get_portal_user_api_key(self, api_key_id: int, portal_user_id: int | None = None) -> dict[str, Any] | None:
        async with self._connect() as db:
            db.row_factory = aiosqlite.Row
            sql = """
    ...
    async def resolve_portal_user_api_key(self, raw_key: str) -> dict[str, Any] | None:
    ...
    async def create_api_key(self, name: str, quota_remaining: int | None) -> tuple[str, dict[str, Any]]:
        raw_key = f"fcs_{secrets.token_urlsafe(32)}"
        key_hash = self._hash_secret(raw_key)
    ...
    async def get_api_key(self, api_key_id: int) -> dict[str, Any] | None:
        async with self._connect() as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute(
                """
    ...
    async def resolve_service_api_key(self, raw_key: str) -> dict[str, Any] | None:
        key_hash = self._hash_secret(raw_key)
        async with self._connect() as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute(
    ...
    async def ensure_api_key_available(self, api_key_id: int) -> tuple[bool, str]:
        ...
        if not api_key:
            return False, "API Key does not exist"
        ...
        if quota_remaining is not None and int(quota_remaining) <= 0:
            return False, "API Key available times exhausted"

        return True, ""

    async def consume_api_key_quota(self, api_key_id: int, session_id: str | None = None) -> tuple[bool, str]:
        if api_key_id <= 0:
            return True, ""
    ...
    async def list_job_logs(self, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
        if self._job_logs_use_redis():
            ...
            service_key_cache: dict[int, dict[str, Any] | None] = {}
    ...
    async def get_api_key_usage_summary(self, api_key_id: int) -> dict[str, Any] | None:
        api_key = await self.get_api_key(api_key_id)
        if not api_key:
            return None
    ...
    async def upsert_cluster_node(
        ...
        if not node:
            raise RuntimeError("Node saving failed")
        return node
    ...
    async def get_cluster_node(self, node_id: int) -> dict[str, Any] | None:
        async with self._connect() as db:
            db.row_factory = aiosqlite.Row
    ...
    async def get_cluster_node_by_name(self, node_name: str) -> dict[str, Any] | None:
        async with self._connect() as db:
            db.row_factory = aiosqlite.Row
    ...
    async def get_cluster_node_by_base_url(self, base_url: str) -> dict[str, Any] | None:
        normalized_url = base_url.strip().rstrip("/")
        async with self._connect() as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute(
    ...
    async def adjust_cluster_node_sessions(
        ...
        *,
        ...
        """Quickly correct the master-side node session count between heartbeats to reduce scheduling lag."""
        if node_id <= 0:
            return
    ...
    async def get_token(self, token_id: int):
        """Compatible with the call of browser_captcha.py; the independent coding service does not maintain business tokens by default."""
        return None
