...
class Config:
    ...
    def _defaults(self) -> dict[str, Any]:
        return {
            ...
            "captcha": {
                ...
                "browser_flow_website_key": RECAPTCHA_SITE_KEY,
    ...
    def _dump_toml(self, data: dict[str, Any]) -> str:
        ...
        top_keys.extend(sorted(k for k in data if k not in self.ORDERED_TOP_SECTIONS))
    ...
    def browser_flow_website_key(self) -> str:
        ...
        if value is not None:
            ...
            if text:
                return text
        configured = str(
            self._get("captcha", "browser_flow_website_key", RECAPTCHA_SITE_KEY)
        ).strip()
        return configured or RECAPTCHA_SITE_KEY
