...
from google_flow.utils.proxy import validate_proxy_format
from .updater import token_syncer

APP_VERSION = "3.3.0"
BASE_DIR = Path(__file__).resolve().parent
STATIC_DIR = BASE_DIR.parent / "api" / "static"
...
def _validate_session_token(session_token: str | None) -> str:
    ...
    if expiry is None or (expiry and expiry <= now):
        ...
        raise HTTPException(401, "Login has expired")
...
def _validate_proxy(proxy_url: str) -> str:
    ...
    if len(clean) > MAX_PROXY_LEN:
        raise HTTPException(400, "Proxy address is too long")
    valid, msg = validate_proxy_format(clean)
    if not valid:
        raise HTTPException(400, f"Agent format error: {msg}")
    return clean
...
def _validate_flow2api_url(flow2api_url: str, *, required: bool = False) -> str:
    ...
    if not clean:
        if required:
            raise HTTPException(400, "Flow2API address cannot be empty")
        return ""
    if len(clean) > MAX_FLOW2API_URL_LEN:
        raise HTTPException(400, "Flow2API address is too long")

    parsed = urlparse(clean)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise HTTPException(400, "The Flow2API address format is incorrect and must be http(s)://host[:port]")
    return clean.rstrip("/")
...
def _validate_connection_token(connection_token: str) -> str:
    ...
    if len(clean) > MAX_CONNECTION_TOKEN_LEN:
        raise HTTPException(400, "The connection token is too long")
    return clean
...
def _validate_login_account(login_account: str) -> str:
    ...
    if len(clean) > MAX_LOGIN_ACCOUNT_LEN:
        raise HTTPException(400, "Login account is too long")
    return clean
...
def _validate_login_password(login_password: str) -> str:
    ...
    if len(clean) > MAX_LOGIN_PASSWORD_LEN:
        raise HTTPException(400, "Login password is too long")
    return clean
...
def _normalize_login_credentials(login_account: str, login_password: str) -> tuple[str, str]:
    ...
    if bool(account) != bool(password):
        raise HTTPException(400, "Login account and password need to be provided at the same time")
...
def _split_import_line(line: str) -> tuple[str, str, str]:
    ...
    raise HTTPException(400, f"Import format error: {line}")


def _parse_account_import_content(content: str) -> list[dict[str, str]]:
    raw = str(content or "")
    if len(raw) > MAX_IMPORT_CONTENT_LEN:
        raise HTTPException(400, "Import content is too large")
    ...
    for line_number, line in enumerate(raw.splitlines(), start=1):
        ...
        try:
            ...
            raise HTTPException(exc.status_code, f"Line {line_number}: {exc.detail}") from exc
        items.append(
            {
                "name": name,
    ...
    if not items:
        raise HTTPException(400, "There is no account that can be imported. Please paste the account password by line.")
    return items
...
def _classify_failure_reason(message: str) -> str:
    ...
    if "Unable to extract token" in lowered or "no token" in lowered:
        return "Token extraction failed"
    if "Log in" in text and ("Expired" in text or "fail" in text):
        return "Login status invalid"
    if "http 401" in lowered or "http 403" in lowered:
        return "Authentication failed"
    if "http" in lowered:
        return "Upstream interface error"
    if "timeout" in lowered or "time out" in text:
        return "Request timeout"
    if "acting" in text or "proxy" in lowered:
...
class CreateProfileRequest(BaseModel):
    ...
    proxy_url: str | None = ""
    flow2api_url: str | None = ""
    connection_token_override: str | None = ""
...
class UpdateProfileRequest(BaseModel):
    ...
    flow2api_url: str | None = None
    connection_token_override: str | None = None
...
def _normalize_cookie_export_kind(kind: str | None) -> str:
    ...
    if normalized in {"google", "protocol"}:
        return "google"
    raise HTTPException(400, "Cookie export type only supports session/google")
...
def _build_google_cookie_export(profile: dict[str, Any]) -> dict[str, Any]:
    ...
    if not raw:
        raise HTTPException(404, "There are no exportable Google Cookies for the current account")

    cookies = _parse_google_cookie_export(raw)
    if not cookies:
        raise HTTPException(400, "The Google Cookies saved in the current account cannot be parsed.")
...
async def launch_browser(profile_id: int, token: str = Depends(verify_session)):
    if not config.enable_vnc:
        raise HTTPException(400, "VNC login disabled (enabled by setting ENABLE_VNC=1)")
    profile = await profile_db.get_profile(profile_id)
    if not profile:
        raise HTTPException(404, "does not exist")
    async with execution_gate.hold(
    ...
    if not success:
        raise HTTPException(500, "Startup failed")
    await dashboard_events.publish("browser_launch", {"profile_id": profile_id})
    return {"success": True, "message": "Please log in via VNC"}
...
async def import_cookies(profile_id: int, request: ImportCookiesRequest, token: str = Depends(verify_session)):
    ...
    if not cookies_json:
        raise HTTPException(400, "Cookie content cannot be empty")
    profile = await profile_db.get_profile(profile_id)
    if not profile:
        raise HTTPException(404, "does not exist")
    async with execution_gate.hold(
...
async def export_cookies(
    profile_id: int,
    kind: str = Query("google", description="session|google, default google"),
    token: str = Depends(verify_session),
):
    profile = await profile_db.get_profile(profile_id)
    if not profile:
    ...
    if not result.get("success"):
        ...
        status_code = 404 if any(marker in error for marker in ("No", "No persistent data", "does not exist", "not found")) else 400
...
async def protocol_login(profile_id: int, request: ProtocolLoginRequest, token: str = Depends(verify_session)):
    ...
    if not google_cookies:
        raise HTTPException(400, "Google Cookies cannot be empty")
    ...
    if not result.get("success"):
        raise HTTPException(400, result.get("error") or "Protocol login failed")
    return result
...
async def auto_login_profile(profile_id: int, token: str = Depends(verify_session)):
    ...
    if not result.get("success"):
        raise HTTPException(400, result.get("error") or "Automatic login failed")
    return result
...
async def extract_token(profile_id: int, token: str = Depends(verify_session)):
    ...
    return {"success": False, "message": "Token not found, please log in first"}
...
async def update_config(request: UpdateConfigRequest, api_request: Request, token: str = Depends(verify_session)):
    ...
    if request.refresh_interval is not None:
        if request.refresh_interval < 1 or request.refresh_interval > 1440:
            raise HTTPException(400, "The refresh interval needs to be between 1-1440 minutes")
    ...
    if request.refresh_interval is not None and config.refresh_interval != old_interval:
        ...
        if scheduler:
            try:
                ...
                logger.warning(f"Failed to update scheduled task: {exc}")
